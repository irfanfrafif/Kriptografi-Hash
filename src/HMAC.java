public class HMAC {
    public static String computeHMAC(String message, String key) {
        int okeypad = HashCipher.computeHash(key, 0xaaaa) ^ 0x5c5c;
        int ikeypad = HashCipher.computeHash(key, 0xaaaa) ^ 0x3636;
        ...
        int innerHash = HashCipher.computeHash(blockss, 0x1234);

        int[] innerHashBlock = { okeypad, innerHash };
        int hmac = HashCipher.computeHash(innerHashBlock, 0x4321);
